...
type t = {
  name : string;
  git_commit : string;
  change_types : change_type list;
  summary : string option;
  details : string option;
}

let make ~name ~git_commit ~change_types ?summary ?details () =
  { name; git_commit; change_types; summary; details }

let name t = t.name
let git_commit t = t.git_commit
let change_types t = t.change_types
...
let jsont : t Jsont.t =
  ...
  |> Jsont.Object.mem "change_types" (Jsont.list change_type_jsont) ~enc:(fun r -> r.change_types)
...
let json_schema =
  ...
  json_object [
    ...
    json_field "properties" (json_object [
      ...
      json_field "change_types" (json_object [
        ...
        json_field "description" (json_string "Types of changes: unchanged (no meaningful diff), janestreet (package from github.com/janestreet or github.com/oxcaml), dune-port (adding dune build support), oxcaml (uses oxcaml features like unboxed types), new-feature (adds new features not in upstream), bugfix (bug fixes not yet in upstream), compatibility (compatibility fixes for monorepo), build-fix (build system fixes beyond dune)")
      ]);
      json_field "summary" (json_object [
        json_field "type" (json_string "string");
        json_field "description" (json_string "Single sentence summary of the changes. Omit if change_types is ['unchanged'] or ['janestreet'].")
      ]);
      json_field "details" (json_object [
        json_field "type" (json_string "string");
        json_field "description" (json_string "Longer markdown description with bullet points detailing specific changes. Omit this field if change_types is ['unchanged'] or ['janestreet'].")
      ])
    ]);
    json_field "required" (json_array [
      json_string "name";
      json_string "git_commit";
      json_string "change_types"
    ])
  ]
